          ...
          <div key={index} className="space-y-3 group">
            ...
            <div className="flex flex-wrap gap-2 ml-4">
              {category.subServices.map((sub, idx) => (
                <Badge
                  key={idx}
                  className="px-3 py-1 text-sm font-medium bg-primary/10 text-primary border border-primary/20 transition-all duration-200 hover:scale-105 hover:bg-primary/20 hover:shadow-sm cursor-default"
                >
                  {sub}
                </Badge>
              ))}
