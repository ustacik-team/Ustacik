      ...
      <CardContent className="space-y-3">
        <div className="grid gap-3 sm:grid-cols-2">
          {checks.map(([Icon, title, detail, enabled]) => (
            <div key={title as string} className="flex gap-3 rounded-xl border border-border/60 bg-background/50 p-3">
              <div className={`mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-full ${enabled ? "bg-emerald-500/10 text-emerald-600" : "bg-muted text-muted-foreground"}`}>
        ...
        <p className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-xs leading-relaxed text-muted-foreground">
          Ustacik connects customers with craftsmen but does not guarantee the work. Please agree on scope, pricing, and timing directly before any work begins.
        </p>
      </CardContent>
    </Card>
  );
