...
export function RequestJobForm({ craftsman, regions }: RequestJobFormProps) {
  ...
  const onSubmit = () => {
    ...
    toast.success(`Your request for ${craftsman.name} was submitted!`);
    setSuccessOpen(true);
  };
    ...
    <>
      <Card className="border-border/70 bg-card/95 shadow-lg shadow-primary/5">
        <CardHeader className="border-b border-border/60">
          ...
          <CardDescription>Fields marked with an asterisk are required. Provide clear details to receive an accurate response.</CardDescription>
        </CardHeader>
        <CardContent className="pt-6">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-7">
              ...
              <Button type="submit" size="lg" className="w-full gap-2 sm:w-auto"><Send className="size-4" />Send job request</Button>
            </form>
          </Form>
        </CardContent>
      </Card>
