  ...
  return (
    <header className="space-y-5">
      <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1.5 text-sm text-muted-foreground">
        ...
        <Link href="/find-craftsmen" className="transition-colors hover:text-primary">
          Craftsmen
        </Link>
        <ChevronRight className="size-3.5" />
        <Link href={`/craftsmen/${craftsmanId}`} className="max-w-40 truncate transition-colors hover:text-primary">
          {craftsmanName}
        </Link>
        <ChevronRight className="size-3.5" />
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
        <Button variant="outline" size="icon" asChild className="shrink-0 self-start rounded-full" aria-label={`Back to ${craftsmanName}'s profile`}>
          <Link href={`/craftsmen/${craftsmanId}`}>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-primary">
            ...
            <span className="text-sm font-semibold">Direct service request</span>
          </div>
          <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Request a job</h1>
          <p className="max-w-2xl text-muted-foreground">
            Share the job requirements with {craftsmanName}. Provide clear details and address for an accurate response.
          </p>
          {category && (
            <p className="text-xs font-medium text-muted-foreground">
              Primary category: <span className="text-foreground">{category}</span>
            </p>
          )}
        </div>
      </div>
